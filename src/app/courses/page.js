...
const CourseList = ({ courses }) => {
  ...
  const handleSaveChanges = () => {
    ...
    // Encontrar o usuário com o email correspondente ao loggedInUser
    ...
    // Atualizar o email do usuário logado no localStorage
  ...
  const handleSubscribe = (materiaId) => {
    ...
    // Adicionar nova inscrição ao localStorage
    const newRegistration = { email, materia_id: materiaId };
    registrations.push(newRegistration);
    localStorage.setItem("registrations", JSON.stringify(registrations));

    // Adicionar o ID do curso ao estado userCourses
  ...
  const handleCancelSubscription = (courseId) => {
    // Remover o curso da lista de cursos matriculados pelo usuário
    const updatedUserCourses = userCourses.filter((id) => id !== courseId);
    setUserCourses(updatedUserCourses);

    // Atualizar o localStorage com a lista de cursos matriculados atualizada
    const loggedInUser = localStorage.getItem("loggedInUser");
    const registrations =
      JSON.parse(localStorage.getItem("registrations")) || [];
